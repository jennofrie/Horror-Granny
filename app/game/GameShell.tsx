...
const INITIAL_HUD: HudState = {
  ...
  stamina: 1,
  prompt: null,
  objective: "FIND A WEAPON — GRANDMA IS HOME",
  flashlight: true,
  sneaking: false,
  cheats: null,
  enemy: null,
...
/** banner subtitle per objective phase (objective text before the dash) */
const OBJECTIVE_HINTS: Record<string, string> = {
  "FIND A WEAPON": "GRAB WHATEVER YOU CAN SWING — OR WHATEVER SHOOTS",
  SURVIVE: "IT WILL COME FOR YOU — PUT IT DOWN",
  "SOMETHING ELSE IS IN THE HOUSE…": "GRANDPA HAS WOKEN UP",
  "IT KNOWS YOUR NAME.": "THE DEVIL WALKS — END THIS",
  "THE DOOR IS OPEN. RUN.": "THROUGH THE FRONT DOOR — GO",
};

/** per-killer death screen titles */
const DEATH_TITLES: Record<string, string> = {
  grandma: "GRANNY GOT YOU",
  grandpa: "GRANDPA GOT YOU",
  devil: "IT TOOK YOU",
};
...
export default function GameShell() {
  ...
  const [stats, setStats] = useState<{ seconds: number; kills: number; enemy: string | null }>({
    seconds: 0,
    kills: 0,
    enemy: null,
  });
  ...
  const callbacksRef = useRef<EngineCallbacks>({
    ...
    onHud: (h) => {
      setHud(h);
      // Big objective banner whenever the objective *phase* changes (run
      // start, armed, tier transitions, door open) — not on every HUD tick.
      ...
      if (kind !== bannerKindRef.current) {
        ...
        setBanner({ title: h.objective, hint: OBJECTIVE_HINTS[kind] ?? "" });
  ...
  /** Tear down the current run and remount a fresh engine. */
    ...
    <div className="fixed inset-0 select-none overflow-hidden bg-black">
      ...
      {isTouch && portrait && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-6 bg-[#0a0905]/97">
          ...
          <p className="font-elite max-w-xs text-center text-xs leading-5 tracking-[0.15em] text-amber-100/40">
            the house only exists in landscape
          </p>
        </div>
      )}
      ...
      {(state === "playing" || state === "dying") && (
        <div className="pointer-events-none absolute inset-0 cursor-none">
          {/* P5 occlusion: peering out of a hiding spot. The wardrobe is
              near-black with a thin door-crack slit; under the bed the
              frame and shadow swallow the top and bottom of the view. */}
          {hud.hidden === "wardrobe" && (
            <div
              className="absolute inset-0"
              style={{
                background:
                  "linear-gradient(90deg, rgba(2,1,0,0.985) 0%, rgba(2,1,0,0.985) 40%, rgba(2,1,0,0.72) 46%, rgba(2,1,0,0.38) 50%, rgba(2,1,0,0.72) 54%, rgba(2,1,0,0.985) 60%, rgba(2,1,0,0.985) 100%)",
              }}
            />
          )}
          {hud.hidden === "under-bed" && (
            <div
              className="absolute inset-0"
              style={{
                background:
                  "linear-gradient(180deg, rgba(2,1,0,0.94) 0%, rgba(2,1,0,0.62) 30%, rgba(2,1,0,0.14) 60%, rgba(2,1,0,0.6) 100%)",
              }}
            />
          )}

          {/* crosshair (dims while hidden — nothing to aim at in here) */}
          <div
            className={`absolute left-1/2 top-1/2 h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full ${
              hud.hidden ? "bg-amber-100/15" : "bg-amber-100/40"
            }`}
          />

          {/* hit marker — brief red × when an attack connects */}
          {hud.hitFlash && (
            <div className="font-elite absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-xl leading-none text-red-400/90 [text-shadow:0_0_10px_rgba(255,60,60,0.7)]">
              ×
            </div>
          )}
          ...
          {/* sneaking / hidden indicator */}
          {hud.hidden ? (
            <div className="font-elite absolute bottom-14 left-1/2 -translate-x-1/2 text-[11px] tracking-[0.4em] text-amber-100/55 [text-shadow:0_0_10px_rgba(255,220,150,0.25)]">
              — HIDDEN · HOLD STILL —
            </div>
          ) : (
            hud.sneaking && (
          ...
          {/* the thing hunting you, or the kill-chain score in the gaps */}
          <div className="font-elite absolute bottom-4 left-5 text-sm tracking-[0.3em] text-amber-100/60">
            {hud.enemy ? `⚠ ${hud.enemy.toUpperCase()}` : `KILLS ${hud.kills}/3`}
          </div>
          ...
          {/* equipped weapon + ammo, slot hints (not while hidden) */}
          {hud.weapon && !hud.hidden && (
            <div className="font-elite absolute bottom-10 right-5 text-right">
              <div className="text-sm tracking-[0.3em] text-amber-100/85 [text-shadow:0_0_12px_rgba(255,220,150,0.35)]">
                {hud.weapon.name}
                {hud.weapon.ammo !== null && (
                  <span className={hud.weapon.ammo === 0 ? "text-red-300/90" : ""}>
                    {" "}• {hud.weapon.ammo}
                  </span>
                )}
              </div>
              {hud.slots && (
                <div className="mt-1 text-[10px] tracking-[0.2em] text-amber-100/35">
                  {hud.slots}
                </div>
              )}
            </div>
          ...
          {showCredits ? (
            <>
              ...
              <div className="font-elite mt-8 flex max-w-md flex-col items-center gap-3 text-center text-[13px] leading-6 tracking-[0.12em] text-amber-100/55">
                <p>a game by Jennofrie</p>
                <p className="text-amber-100/35">
                  the house, its textures, the engine and the post effects are
                  generated by code. the people it eats — models, music and
                  screams — come from CC0 / CC-BY artists, credited in
            ...
            <>
              ...
              <p className="font-elite mt-6 max-w-md text-center text-sm leading-6 text-amber-100/45">
                find a weapon. kill what hunts you. get out. grandma is home
                — so is grandpa, and the thing they answer to. the front door
                only opens when the devil is dead.
              </p>
              ...
              {isTouch ? (
                <div className="font-elite mt-7 grid grid-cols-2 gap-x-10 gap-y-1.5 text-[12px] tracking-[0.2em] text-amber-100/35">
                  ...
                  <span>BUTTONS — TORCH / SNEAK / FIRE</span>
                </div>
              ) : (
                <div className="font-elite mt-7 grid grid-cols-2 gap-x-10 gap-y-1.5 text-[12px] tracking-[0.2em] text-amber-100/35">
                  <span>WASD — WALK</span>
      ...
      {state === "paused" && (
        <Overlay>
          ...
          <p className="font-elite mt-3 text-[10px] tracking-[0.25em] text-amber-100/20">
            THE RUN IS LOST. THE HOUSE KEEPS IT.
          </p>
...
function GitHubBadge({
  ...
  return (
    <a
      href="https://github.com/Jennofrie/Horror-Granny"
      target="_blank"
      ...
      <div className="absolute bottom-10 right-5 flex flex-col items-end gap-3">
        {weapon && (
          <div className="flex gap-3">
            <button
              className="font-elite rounded border border-amber-100/30 bg-black/30 px-4 py-3 text-[11px] tracking-[0.2em] text-amber-100/70"
              onPointerDown={() => engineRef.current?.touchCycleWeapon()}
            >
              WEAPON
            </button>
            <button
              className="font-elite rounded border border-red-300/50 bg-red-400/15 px-7 py-3.5 text-sm tracking-[0.25em] text-red-100"
              onPointerDown={() => engineRef.current?.touchAttack()}
            >
              FIRE
            </button>
          </div>
